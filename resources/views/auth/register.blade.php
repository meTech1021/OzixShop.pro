...
<div class="content">
	...
	<form action="{{ route('auth.signup') }}" method="post" id="signup_form">
        @csrf
		<h3 class="bold text-center">Create An Account</h3><br>
		<div class="form-group">
			<label class="control-label visible-ie8 visible-ie9">Username</label>
			<div class="input-icon">
				<i class="fa fa-user"></i>
				<input class="form-control placeholder-no-fix" type="text" autocomplete="off" placeholder="Username" id="username" value="{{ old('name') }}" name="name"/>

			</div>
            @if($errors->has('name'))
            <span class="form-text text-danger" id="username_error">{{ $errors->first('name') }}</span>
            @endif
		</div>
        <div class="form-group">
