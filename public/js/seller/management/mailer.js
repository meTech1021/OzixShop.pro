$(document).ready(function() {
    var TableAdvanced = function () {
        ...
        var mailer_table = function () {

            var table = $('#mailer_table');
            ...
            table.on('click', '.btn_remove', function() {
                ...
                $.ajax({
                    url : '/seller/management/mailer_delete',
                    method : 'post',
                    data : {
            ...
            $('#btn_save').click(function() {
                var mailer_host = $('#mailer_host').val();
                var price = $('#price').val();

                if(mailer_form.valid()) {
                    Metronic.blockUI({
                        target: '.modal-content',
                        animate: true
                    });
                    $.ajax({
                        url : '/seller/management/mailer_save',
                        method : 'post',
                        data : {
                            ...
                            if(data.msg === 'success') {
                                ...
                                if(data.not_working_hosts.length > 0) {
                                    for(var i = 0 ; i < data.not_working_hosts.length; i ++) {
                                        ...
                                        $('#mailer_host').closest('.form-group').addClass('has-error');
                                    }
                                }
                                ...
                                $('#mailers_badge').text(data.mailer_cnt);
                                $('#mailer_cnt').text(data.mailer_cnt);
                            }
